...
export default function InventoryView() {
  ...
  useEffect(() => {
    ...
    .then(async ([recipesData, menusData, fridgeData, groceryListsData]) => {
...
/>
  ...
  <div className="flex gap-4">
    ...
    >
      Need to Get ({getFilteredInventory().length})
    ...
    switch (inventoryFilter) {
      case 'inStock':
        ...
        case 'needed':
          ...
          const groceryListItems = selectedList.items
            .map((item) => item.name.replace(/[*#]/g, '').trim())
            .filter((name) => name !== '');
    
          const neededItems = groceryListItems.filter(
            (name) => !fridgeItems.some((item) => item.name === name && item.quantity > 0)
          );
    
          const newFridgeItems = neededItems
            .filter((name) => !fridgeItems.some((item) => item.name === name))
            .map((name) => ({ name, quantity: 0, unit: '' }));
    
          setFridgeItems([...fridgeItems, ...newFridgeItems]);
    
          return [...fridgeItems, ...newFridgeItems].filter(
            (item) => item.quantity === 0 && groceryListItems.includes(item.name)
          );
      case 'all':
        return fridgeItems;
      default:
  ...
  <div className="max-w-6xl mx-auto px-4 py-4">
    ...
    >
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        setSelectedGroceryList={setSelectedGroceryList}
